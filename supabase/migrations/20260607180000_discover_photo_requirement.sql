-- Discover feed: treat gallery photos the same as avatar_url (matches client primaryProfilePhotoUrl).
...
begin
  ...
  where p.id <> p_viewer_id
    ...
    and coalesce(
      ...
      (
        select nullif(trim(u), '')
        from unnest(coalesce(p.profile_photos, '{}')) as u
