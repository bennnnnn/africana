-- Discover feed: server-side block/like exclusion (avoids giant NOT IN URLs).
...
begin
  ...
  where p.id <> p_viewer_id
    ...
    and (p.state is null or p.state = p_state)
...
revoke all on function public.fetch_discover_profiles_page(
  uuid, text, int, int, text, text, text, text, boolean, boolean, boolean, int, int
) from public;
grant execute on function public.fetch_discover_profiles_page(
  uuid, text, int, int, text, text, text, text, boolean, boolean, boolean, int, int
) to authenticated;
